...
//var triColor = "rgba(255, 255, 255, 0.5)";
...
function Type( args ) {
	...
	this.shapeColor = args.shapeColor;
	this.input = args.input;
	this.populationSize = args.populationSize;
	this.initialize();
}

Type.prototype.initialize = function() {
	for ( var i = 0; i < this.input.length; i++ ) {
		var workingCtx = getWorkingCtx( i, this.input.length );
		var char = this.input.charAt( i );
		var pixelData = getTargetData( i, char, this.backgroundColor, this.shapeColor );
		pops.push( new Population( this.populationSize, numShapes, workingCtx, pixelData, this.backgroundColor, this.shapeColor ) );
	}
	cycle = 0;
	generation = 0;
	evolve();
...
function display() {
	...
	for ( i = 0; i < numShapes; i++ ) {
		...
		pops[ popIndex ].ctx.fillStyle = pops[ popIndex ].shapeColor; //'rgba(' + t.r + ',' + t.g + ',' + t.b + ',' + 0.3 + ')'; <-- for multicolor image
...
function clear() {
	//console.log('-->',  pops[popIndex]);
	pops[ popIndex ].ctx.fillStyle = pops[ popIndex ].backgroundColor;
...
function fitness() {
	...
	for ( var i = 0; i < canvasWidth * canvasHeight * 4; i += 4 ) {
		...
		fit += ( greenFit * greenFit ) + ( blueFit * blueFit ); //+ (alphaFit * alphaFit * 255); //add alphaFit (redFit * redFit); //+
	}
...
function Population( popSize, numShapes, context, targetPixelData, backgroundColor, shapeColor ) {
	this.genomes = [];
	for ( var i = 0; i < popSize; i++ ) {
